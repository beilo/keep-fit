...
type TOption = Omit<Option, "url"> & { url: string | string[], params?: string | object };
...
const handleUrlParams = (url: TOption['url'], params: TOption['params']) => {
...
export const navigateTo = (opt: TOption) => {
    Taro.navigateTo({...opt, url: handleUrlParams(opt.url, opt.params)});
};

export const redirectTo = (opt: TOption) => {
    Taro.redirectTo({...opt, url: handleUrlParams(opt.url, opt.params)});
};
